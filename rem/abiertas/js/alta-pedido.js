...
$(document).ready(function(){
  ...
  $(document).on('change','#empresa',function(e) {
    ...
    //Verificamos si necesita alguna leyenda
    var empresa = $(this).val();
    if(empresa != '3G Consulting y Asesoría') {
      //En los que no son 3G Consulting y Asesoría NO se ve IMPORTE
      var html_legend = '<small>*El campo de IMPORTE no se ve en la remisión esta empresa.</small>';
    } else {
      var html_legend = '';
    }

    $('.cont-legend').html(html_legend);
...
function getFolio() {
    ...
    $.ajax({
        type: 'POST',
        url: 'include/Libs.php?accion=getFolio',
        dataType:'json',
        data: params,
        error: function(){
          bootbox.dialog({
