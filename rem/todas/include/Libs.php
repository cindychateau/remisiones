		...
		try {
			...
			*/
			...
			foreach ($puntero as $row) {
				...
				$btn_imprimir= '<a class="btn-action" target="_blank" href="../abiertas/include/print.php?id='.$row['id'].'" data-id="'.$row['id'].'">
								<button type="button" class="btn btn-blue"><i class="fa ft-printer"></i>Imprimir con Fondo</button>
							   </a>';	
				$btn_imprimir.= '<a class="btn-action" target="_blank" href="../abiertas/include/print.php?id='.$row['id'].'&fondo=0" data-id="'.$row['id'].'">
				...
				if($row['estatus'] == 'Abierta') {
					$estatus = '<span class="badge badge-success">Abierta</span>';
				} else if($row['estatus'] == 'Cerrada') {
					$estatus = '<span class="badge badge-danger">Cerrada</span>';
				} else {
					$estatus = '<span class="badge badge-primary">'.$row['estatus'].'</span>';
				}

				$aRow = array($row['folio'],$row["SIU_NOMBRE"], $row['shortname'], $row['nombre'], $row['empresa'],$estatus,$row['servicio'], $row['ocs'], $row['fecha'], $row['fecha_cerrada'], $row['direccion'], $tabla_info, $btn_imprimir);
		...
		//if(isset($_POST['siu_id'])) {
			...
			try {
				...
				foreach ($result as $row) {
					...
					$json['cliente_select'] .= '<option value="'.$row['shortname'].' - '.$row['empresa'].'">'.$row['shortname'].' - '.$row['empresa'].'</option>';
				}
			...
			if(!empty($_POST['motivo'])) {
	            ...
	            $body = '<table border="0" cellpadding="0" cellspacing="0" width="600">
							<div style="width: 600px; padding: 20px 40px; font-family: sans-serif; font-size: 14px;">
								...
								<div style="padding: 20px 25px; background-color: #125dab; color: white; text-align: center; font-size: 16px;">
									<b>
										Para realizar el cambio solicitado ingrese al siguiente enlace: '.$url_solicitud.'cerrar.php?cs='.$clave_secreta.'&r='.$_POST['id'].'<br><br>
