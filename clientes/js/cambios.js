...
$(document).ready(function(){
	...
	$(document).on('click','.eliminar-contacto-exist',function(e) {
		e.preventDefault();
		var id = $(this).attr("data-id");
		var html_elim = '<input name="contacto_elim_id[]" type="hidden" value="'+id+'">';
		$('#cont-contacto').append(html_elim);

		$(this).closest('.row-contacto').remove();
	});
	...
	$.ajax({
		...
		success: function(result){
			if(!result.error){
				...
				$('#cont-contacto').html(result.html_contactos);
				contacto = result.num_contactos;
			}
			else {
				window.location = "index.php";
			}
