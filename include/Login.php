...
class Login extends Core
{
	...
	*/
	public function log_in($log = false) {
		...
		$credentials = array('SIU_NOMBRE','SIU_ID', 'SUP_ID', 'SIU_EMAIL');
		...
		if (empty($json['msg'])) {
			...
			if ($log && $json['auth']) {
				session_start();
				$_SESSION["gm_adm"]["username"] = $json['credentials']['SIU_NOMBRE'];
				$_SESSION["gm_adm"]["userid"] = $json['credentials']['SIU_ID'];
				$_SESSION["gm_adm"]["userprofile"] = $json['credentials']['SUP_ID'];
				$_SESSION["gm_adm"]["email"] = $json['credentials']['SIU_EMAIL'];
